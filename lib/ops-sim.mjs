...
export function deployMission(world, title, now = world.now, extras = {}) {
  const next = clone(world);
  next.now = now;
  next.running = true;
  next.tick += 1;
  next.mission = {
    ...
    reward: String(extras.reward || "1,200 USDT").trim() || "1,200 USDT",
    brief: String(extras.brief || "").trim(),
    criteria: String(extras.criteria || "").trim(),
    progress: 8,
    phaseIndex: 0,
    status: "active",
    startedAt: now,
