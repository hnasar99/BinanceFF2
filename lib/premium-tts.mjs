import { engineLabel, engineRank, speakableText, voiceProfile } from "./agent-voice.mjs";
...
export async function synthesizeSpeech({
  ...
  const keys = voiceSecretsFrom(secrets);
  const errors = [];
  const allow = (engine) => !betterThan || engineRank(engine) > engineRank(betterThan);

  const run = async (engine, task) => {
    if (!allow(engine)) return null;
    try {
      return await task();
    } catch (error) {
  ...
  throw new Error(betterThan ? "no_upgrade" : errors.join(" | ") || "neural_tts_failed");
}
